...
import br.com.engenharia.projeto.ProjetoFinal.entidades.estoque.EstoqueServiceExcecao;
...
public class ServicePedido {
	...
	private void verificaDiponibilidadeEmEstoque(DadosCadastroPedido dados, Long livroId) {
		...
		if(dados.quantidade() > disponivel) {
			throw new EstoqueServiceExcecao("Quantidade requerida superior a disponivel em estoque");
		}
	}
	...
	private Livro verificaExistenciaLivro(Long livroId) {
		...
		if(livroId == null && livro.isEmpty()) {
			throw new LivroServiceExcecao("Livro não encontrado");
		}
		return livro.get();
	}
}